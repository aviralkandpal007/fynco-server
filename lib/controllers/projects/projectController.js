...
class ProjectController {
  ...
  async create(req, res) {
    try {
      ...
      if (!name) {
        return ResponseHandler.sendBadRequest(res, 'Project name is required')
      }

      if (!Object.values(PROJECT_STATUS).includes(statusCode)) {
        return ResponseHandler.sendBadRequest(res, 'Invalid project status')
      }
      if (deadline && !parsedDeadline) {
        return ResponseHandler.sendBadRequest(res, 'Invalid deadline')
      }
      if (parsedDeadline) {
        const today = new Date()
        today.setHours(0, 0, 0, 0)
        if (parsedDeadline.getTime() < today.getTime()) {
          return ResponseHandler.sendBadRequest(res, 'Deadline cannot be in the past')
        }
      }
      ...
      return ResponseHandler.sendCreated(res, { project }, 'Project created')
    } catch (_err) {
  ...
  async list(req, res) {
    try {
      ...
      return ResponseHandler.sendSuccess(res, { projects }, 'Projects fetched')
    } catch (_err) {
      return ResponseHandler.sendServerError(res, 'Server error')
    }
  }
  ...
  async addTaskStatus(req, res) {
    try {
      ...
      if (!label) {
        return ResponseHandler.sendBadRequest(res, 'Status label is required')
      }

      const project = await ProjectModel.findByIdForOwner(projectId, userId)
      if (!project) {
        return ResponseHandler.sendNotFound(res, 'Project not found')
      }

      const statusExists = (project.taskStatuses || []).some((item) => item.label.toLowerCase() === label.toLowerCase())
      if (statusExists) {
        return ResponseHandler.sendConflict(res, 'Status already exists for this project')
      }
      ...
      return ResponseHandler.sendSuccess(res, { project: updatedProject }, 'Project status added')
    } catch (_err) {
      return ResponseHandler.sendServerError(res, 'Server error')
    }
  }
